<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Document</title>
    <link rel="stylesheet" type="text/css" href="css/form_exito_error.css">
</head>
<body>
    <table>
        <tr>
            <td><h2>Error de conexión</h2></td>
        </tr>
        <tr>
            <td><p>Vuelvelo a intentar y si el error persiste, puedes ponerte en contacto directamente con la persona encargada de llevar a cabo tu pedido en info@deoroverde.szmblog.es</p></td>
        </tr>
        <tr>
            <td>¡Gracias!</td>
        </tr>
        <tr>
        <td class="boton"><a class="volver" href="realizar_pedido.php">Volver</a></td>
        </tr>
    </table>
</body>
</html>